  ...
  .done(function(data) {
    ...
    $.ajax({
      ...
      .done(function(data) {
        ...
        // An input for the user to add a title to their comment
        $(".comment").append("<form><div class='form-group'><label for='titleinput'>Note Title: </label>"
        + "<input class='form-control' id='titleinput' type='text' placeholder='Add a title to your comment'></div>" 
        // a textarea for the user to type in their comment 
        + "<div class='form-group'><label for='bodyinput'>Share your opinion: </label>"    
        + "<textarea class='form-control' id='bodyinput' rows='5' placeholder='Type your comment here'></textarea><div>"
        + "<button data-id='" + data._id + "' id='savecomment'>Post and post</button></form>");
...
// When the user clicks on the Read Note link
    ...
    .done(function(data) {
      ...
      if (data.note.length > 0) {
        for (var i = 0; i < data.note.length; i++) {
          ...
          + "<div class='card-body'> <h4 class='card-title'>" + data.note[i].noteTitle + "</h4>"
          + "<p class='card-text'>" + data.note[i].noteBody + "</p>"
          + "<hr><p>" + data.note[i].userAssociation + "</p></div></div>");
        }
      }
      else {
        $(".read").append("<p>Go ahead and comment here!</p>");
      }
    });
});
